...
import { ArrowLeft, FileText, Package, Truck, Repeat, CheckCircle, Store, BadgeCheck, DollarSign, User, Phone, MapPin, Copy, Download, RefreshCw, Link as LinkIcon } from "lucide-react";
                            ...
                            <div key={order.id} className="border rounded-lg p-4 space-y-4">
                                ...
                                <div className="flex items-start gap-4">
                                  <div className="relative h-24 w-24 flex-shrink-0 bg-muted rounded-md overflow-hidden flex items-center justify-center">
                                    {isXerox ? (
                                        isDriveFile ? <FileText className="h-12 w-12 text-blue-500" /> : <Package className="h-12 w-12 text-muted-foreground" />
                                    ) : order.productImage ? (
                                 ...
                                 {isXerox && (order.productImage ? (<Button asChild variant="outline" size="sm"><a href={order.productImage} target="_blank" rel="noopener noreferrer"><LinkIcon className="mr-2 h-4 w-4"/> View Document</a></Button>) : (<Badge variant="destructive">Document not uploaded by user.</Badge>))}
                                {isXerox && xeroxDetails.length > 0 && (
                                    <Card className="bg-muted/50">
                                        <CardHeader className="p-2 pb-0">
                                            <CardTitle className="text-sm">Print Configuration</CardTitle>
                                        </CardHeader>
                                        <CardContent className="p-2 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
                                            <div>
                                                <h4 className="font-semibold text-xs mb-1">Configuration</h4>
                                                <Table>
                                                    <TableBody>
                                            ...
                                            <div>
                                                <h4 className="font-semibold text-xs mb-1">Price Estimation</h4>
                                                <Table>
                                                  <TableBody>
                                                    {bindingCost > 0 && (
                                                      <TableRow className="border-0">
