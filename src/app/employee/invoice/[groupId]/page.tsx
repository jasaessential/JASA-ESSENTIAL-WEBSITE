...
export default function EmployeeInvoicePage() {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            if (!groupId) {
                toast({ variant: 'destructive', title: 'Error', description: 'Missing order group information.' });
                return;
            }
            setIsLoading(true);
            try {
                const [fetchedOrders, allUsers, fetchedShops] = await Promise.all([
                ...
                if (validOrders.length > 0) {
                    const orderUser = allUsers.find(u => u.uid === validOrders[0]?.userId);
                    setCustomer(orderUser || null);
                    setSelectedItems(validOrders.map(o => o.id));
                }
    ...
    if (!orders.length || !customer) {
        return <div className="container mx-auto p-8 text-center">No order details found.</div>;
    }
                    ...
                    <Table className="w-full text-xs">
                        <TableHeader>
                            <TableRow className="bg-gray-100">
                                ...
                                <TableHead className="p-1">Seller</TableHead>
                        ...
                        <TableBody>
                            {itemsToRender.map(order => {
                                const shop = allShops.find(s => s.id === order.sellerId);
                                return (
                                    <TableRow key={order.id} className="[&>td]:p-1">
                                        ...
                                        <TableCell className="text-gray-500">{shop?.name || 'N/A'}</TableCell>
        ...
        <div id="invoice-page-container" className="bg-gray-100 dark:bg-gray-900 min-h-screen">
            <div className="container mx-auto p-4 md:p-8">
                 ...
                 <Button asChild className="mb-4 no-print bg-blue-600 text-white hover:bg-blue-700">
                   <Link href={`/employee-dashboard/${groupId}`}>
