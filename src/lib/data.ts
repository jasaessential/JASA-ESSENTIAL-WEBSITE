...
// --- Query/Feedback Functions ---
export const addQuery = async (data: Omit<SiteQuery, 'id' | 'createdAt'>): Promise<string> => {
    try {
...
export const getQueries = async (): Promise<SiteQuery[]> => {
    try {
        const q = query(queriesCollection, orderBy('createdAt', 'desc'));
        const querySnapshot = await getDocs(q);
        return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as SiteQuery));
    } catch (error) {
...
// --- Site Settings Functions (for Contact Info) ---
export const getContactInfo = async (): Promise<ContactInfo> => {
    try {
        const docRef = doc(siteSettingsCollection, 'contact');
        ...
        return {
            address: '2/3 line medu pension line 2 nd street, line medu, Salem 636003',
            phone: '+917418676705',
            email: 'jasaessential@gmail.com',
...
export const updateContactInfo = async (data: Partial<ContactInfo>): Promise<void> => {
    try {
        const docRef = doc(siteSettingsCollection, 'contact');
...
// Lean function to get only product image URLs
export const getAllProductImageUrls = async (): Promise<string[]> => {
  try {
    ...
    querySnapshot.forEach(doc => {
      ...
      if (data.imageNames && Array.isArray(data.imageNames)) {
        urls.push(...data.imageNames);
      }
    });
    return urls;
  } catch (error) {
    console.error("Error getting all product image URLs: ", error);
    throw new Error("Failed to fetch product image URLs.");
  }
};

// Lean function to get only homepage image URLs
export const getAllHomepageImageUrls = async (): Promise<string[]> => {
    try {
        const content = await getHomepageContent();
        if (!content) return [];
        
        const urls: string[] = [];
        if (content.welcomeImageUrl) urls.push(content.welcomeImageUrl);
        
        if (content.categoryImages) {
            Object.values(content.categoryImages).forEach(url => {
        ...
        if (content.banners) {
            content.banners.forEach(banner => {
                if (banner.imageUrl) urls.push(banner.imageUrl);
            });
        }
        return urls;
    } catch (error) {
        console.error("Error getting homepage image URLs: ", error);
        throw new Error("Failed to fetch homepage image URLs.");
    }
}

// Lean function to get only paper sample image URLs
export const getAllPaperSampleImageUrls = async (): Promise<string[]> => {
    try {
        const samples = await getPaperSamples();
        const urls: string[] = [];
        samples.forEach(sample => {
            if (sample.imageUrls && Array.isArray(sample.imageUrls)) {
                urls.push(...sample.imageUrls);
            }
        });
        return urls;
    } catch (error) {
        console.error("Error getting paper sample image URLs: ", error);
        throw new Error("Failed to fetch paper sample image URLs.");
    }
};

// Lean function to get only order image URLs and status
export const getAllOrderImageUrls = async (): Promise<{productImage: string; status: OrderStatus}[]> => {
    try {
        const q = query(ordersCollection, where('productImage', '!=', null));
        const querySnapshot = await getDocs(q);
        ...
        console.error("Error getting all order image URLs: ", error);
        throw new Error("Failed to fetch order image URLs.");
    }
};
...
export const getBrands = async (category?: Brand['category']): Promise<Brand[]> => {
    try {
        ...
        return querySnapshot.docs.map(doc => {
            const data = doc.data();
            const createdAt = data.createdAt?.toDate ? data.createdAt.toDate().toISOString() : null;
            return { id: doc.id, ...data, createdAt } as Brand;
        });
    } catch (error) {
...
export const getAuthors = async (): Promise<Author[]> => {
    try {
        ...
        return querySnapshot.docs.map(doc => {
            const data = doc.data();
            const createdAt = data.createdAt?.toDate ? data.createdAt.toDate().toISOString() : null;
            return { id: doc.id, ...data, createdAt } as Author;
        });
    } catch (error) {
...
export const getProductTypes = async (category?: ProductType['category']): Promise<ProductType[]> => {
    try {
        ...
        return querySnapshot.docs.map(doc => {
            const data = doc.data();
            const createdAt = data.createdAt?.toDate ? data.createdAt.toDate().toISOString() : null;
            return { id: doc.id, ...data, createdAt } as ProductType;
        });
    } catch (error) {
...
export const addXeroxOption = async (type: XeroxOptionType, optionData: Partial<Omit<XeroxOption, 'id' | 'createdAt'>>): Promise<XeroxOption> => {
  ...
  if (type === 'paperType') {
    newOptionData.priceBwFront = optionData.priceBwFront || 0;
    newOptionData.priceBwBoth = optionData.priceBwBoth || 0;
    newOptionData.priceColorFront = optionData.priceColorFront || 0;
    newOptionData.priceColorBoth = optionData.priceColorBoth || 0;
...
export const updateOrderSettings = async (settings: OrderSettings): Promise<void> => {
  try {
    const docRef = doc(orderSettingsCollection, 'config');
    // Sanitize rules: ensure 'to' is null for infinity, and convert strings to numbers
    const sanitizedSettings = {
        itemDeliveryRules: settings.itemDeliveryRules.map(r => ({ ...r, to: r.to === null ? null : Number(r.to) })),
        xeroxDeliveryRules: settings.xeroxDeliveryRules.map(r => ({ ...r, to: r.to === null ? null : Number(r.to) })),
    };
...
export const createOrder = async (orderData: Omit<Order, 'id' | 'createdAt' | 'tracking'>): Promise<string> => {
    try {
        ...
        const orderId = generateShortId(7);
...
export const updateOrderStatus = async (orderId: string, status: OrderStatus, reason?: string): Promise<void> => {
    try {
        ...
        await runTransaction(db, async (transaction) => {
            ...
            if (status === 'Processing' && orderData.status === 'Cancelled') {
                throw new Error("This order has been cancelled by the user and cannot be confirmed.");
            }
            ...
            switch(status) {
              ...
              case 'Replacement Completed': updates['tracking.replacementCompleted'] = now; break;
              ...
              case 'Replacement Confirmed': 
                updates['tracking.replacementConfirmed'] = now; 
                updates['tracking.packed'] = null;
                updates['tracking.shipped'] = null;
                updates['tracking.outForDelivery'] = null;
                updates['tracking.delivered'] = null;
                updates['tracking.replacementCompleted'] = null;
                break;
              ...
              case 'Pending Delivery Confirmation': updates['tracking.outForDelivery'] = now; break;
              case 'Pending Return Confirmation': updates['tracking.pickedUp'] = now; break;
              case 'Pending Replacement Confirmation': updates['tracking.outForDelivery'] = now; break;
            }

            transaction.update(orderDocRef, updates);
            
            // Create notification for customer for specific statuses
            const notificationStatuses: OrderStatus[] = [
                "Processing", "Packed", "Shipped",
                "Rejected", "Return Approved", "Return Rejected", 
                "Replacement Confirmed"
            ];
            
            const confirmationStatuses: OrderStatus[] = [
                "Pending Delivery Confirmation", "Pending Return Confirmation", "Pending Replacement Confirmation"
            ];

            const notificationsSubcollectionRef = collection(db, 'users', orderData.userId, 'notifications');

            if (notificationStatuses.includes(status)) {
                const notificationDocRef = doc(notificationsSubcollectionRef); // Creates a new doc ref in the subcollection
            ...
            if (confirmationStatuses.includes(status)) {
                ...
                const notification: Omit<Notification, 'id'> = {
                    ...
                    message: `Your item "${orderData.productName}" is pending confirmation. Please go to the 'Confirmations' tab in your notifications to confirm.`,
                    sellerMobileNumbers: shopData?.mobileNumbers || [],
                    isRead: false,
                    createdAt: serverTimestamp(),
                };
...
export const markDeliveryFeeAsPaid = async (groupId: string): Promise<void> => {
    try {
        const q = query(ordersCollection, where('groupId', '==', groupId));
        const querySnapshot = await getDocs(q);
        
        if (querySnapshot.empty) {
            throw new Error("No orders found for this group.");
        }

        const batch = writeBatch(db);
        querySnapshot.forEach(doc => {
            batch.update(doc.ref, { isDeliveryFeePaid: true });
        });

        await batch.commit();
    } catch (error) {
        console.error("Error marking delivery fee as paid:", error);
        throw new Error("Failed to update delivery fee status.");
    }
};
...
export const getNotificationsForUser = async (userId: string): Promise<Notification[]> => {
    try {
        const notificationsSubcollectionRef = collection(db, 'users', userId, 'notifications');
        const q = query(notificationsSubcollectionRef, orderBy('createdAt', 'desc'));
        const querySnapshot = await getDocs(q);
...
export const approveReplacement = async (orderId: string): Promise<void> => {
    await updateOrderStatus(orderId, "Replacement Confirmed");
};
